...
test_dataset = create_dataset(read_tsv(os.path.join('/data/xxp/backdoor/HiddenKiller/data/clean/sst-2/test.tsv')))
...
for i in range(epoch):
    ...
    with torch.no_grad():
        ...
        if acc > best_accuracy:
            best_accuracy = acc
            promptModel.plm.save_pretrained("/data/xxp/backdoor/UIT/data/fine_tuned_model")
            tokenizer.save_pretrained("/data/xxp/backdoor/UIT/data/fine_tuned_model")
            print(f"chechpoint saved!")
        print(f"Epoch {epoch + 1}/{epoch}, Test Accuracy: {acc}")

# After training completes, the best model will be saved in the specified directory.
